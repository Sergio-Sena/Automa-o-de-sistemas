...
def automacao_sistema_web():
    ...
    sleep(2)
    # Forçar reload para limpar cache
...
def cadastrar_produtos_csv():
    ...
    with open('produtos_ecommerce.csv', 'r', encoding='utf-8') as arquivo:
        ...
        for produto in reader:
            ...
            # Clicar e preencher nome (foco triplo garantido)
            print(f"  → Preenchendo nome: {nome}")
            pyautogui.click(x_nome, y_nome, duration=1)
            sleep(1)  # Aguardar foco
            pyautogui.click(x_nome, y_nome, duration=0.5)  # Clique duplo
            sleep(0.3)
    ...
    # Fazer logout APENAS após completar todos os 20 produtos
    if contador == 20:
        print("Fazendo logout após completar 20 produtos...")
